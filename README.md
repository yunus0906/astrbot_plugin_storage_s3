...
- 支持指令组 [`/s3 download {fileID}`](README.md) 直接发送对象视频
...
插件会调用 S3 兼容 API 生成对象的预签名下载链接，并使用 [`Video.fromURL()`](main.py:18) 直接把该视频发送出去。
如需同时返回可直接访问的普通文件 URL，请正确配置 [`_conf_schema.json`](_conf_schema.json) 中的 `s3.public_base_url`，否则 `URL` 字段会退回为 `s3://bucket/key` 形式，仅作标识用途，不能直接给客户端下载。
